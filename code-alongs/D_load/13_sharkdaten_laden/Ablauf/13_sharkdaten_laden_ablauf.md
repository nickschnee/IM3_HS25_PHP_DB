...
> **Ziel:** Die 17 Ranking-Zeilen aus Code-Along 10 stehen in einer Tabelle.
> Unterwegs klären sich drei Fragen, die beim Hitzesommer nicht auftauchten.
...
| --- | --- | --- |
| Wie viele Tabellen? | zwei, mit Fremdschlüssel | eine |
...
Standardvorgehen, das man auswendig lernt. Es gibt vier Fragen an die Daten.
...
> statt der mitgelieferten verwenden. Solange die vier Felder `dimension`,
> `rank`, `category` und `incidents` stimmen, läuft `load.php` unverändert.
> Das ist der beste verfügbare Beweis dafür, wozu ein Datenvertrag gut ist –
> und ein guter Moment, ihn genau so zu benennen.

## Vor dem Code (10')

### Eine oder zwei Tabellen?

Erst fragen, dann `schema.sql` zeigen. Beim Hitzesommer haben wir die Städte
ausgelagert – warum hier nicht die `dimension`?

Die Antwort liegt in den Zahlen: 17 Zeilen, zwei verschiedene Werte. Eine
zweite Tabelle bringt hier nichts und macht jede spätere Abfrage länger. Die
Auslagerung beim Hitzesommer war keine Regel, sondern eine Rechnung.
...
`schema.sql` in phpMyAdmin ausführen. Die `UNIQUE`-Zeile am Schluss kurz
benennen, aber noch nicht erklären – sie kommt bei Schritt 5 zum Zug.

## Schritte im Code (25')

`load.php` enthält sieben TODO-Marken:

1. `header('Content-Type: text/plain; charset=utf-8')`.
2. `require __DIR__ . '/../../../config.php';`
3. Transform-Ergebnis holen, `$result['data']` nehmen. Hier die Frage stellen:
   Warum kommen `questions`, `rules` und `limits` nicht in die Datenbank?
4. Verbindung im `try`-Block aufbauen.
5. `$pdo->exec('DELETE FROM shark_rankings')` vor die Schleife.
6. `prepare()` vor der Schleife, `execute()` darin – und dabei
   `'rank_position' => $row['rank']`.
7. Kontrolle: `SELECT COUNT(*)` und pro `dimension` die ersten drei Plätze.
...
- `load.php` im Browser aufrufen: 17 geschriebene Zeilen, danach beide
  Ranglisten mit ihren ersten drei Plätzen.
- In phpMyAdmin nachsehen: 17 Zeilen, zwei `dimension`-Werte.
- **Ein zweites Mal aufrufen.** Es bleiben 17 Zeilen.
